...
public class ATM {
    ...
    // Возвращает количестов денег в банкомате(его баланс)
    ...
    public double getCash(double amount) throws NoCardInserted, NotEnoughMoneyOnCardException, NotEnoughMoneyInATMExeption {
        checkCard();
        ...
        if (account.getBalance() < amount) {
            ...
            this.moneyInATM -= account.withdraw(amount);
            return account.getBalance();
        }

    }

    private void checkCard() throws NoCardInserted {
        if (this.card == null || card.isBlocked()) {
